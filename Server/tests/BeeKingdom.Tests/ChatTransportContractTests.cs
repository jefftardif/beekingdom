...
public sealed class ChatTransportContractTests
{
    ...
    public async Task Capabilities_and_authoritative_validators_share_the_same_limits()
    {
        ...
        Assert.Multiple(()=>{Assert.That(capabilities.Provider,Is.EqualTo("server"));Assert.That(capabilities.ProtocolVersion,Is.EqualTo("chat-v1"));Assert.That(capabilities.Limits.BodyMaxCharacters,Is.EqualTo(5));Assert.That(capabilities.Limits.MaxPrivateRecipients,Is.EqualTo(2));Assert.That(capabilities.Limits.MessagesPerMinutePerPlayer,Is.EqualTo(7));Assert.That(capabilities.ReadCursors,Is.True);Assert.That(capabilities.ModerationReports,Is.True);Assert.That(capabilities.Channels,Is.EqualTo(new[]{ChatChannelType.Alliance,ChatChannelType.Server,ChatChannelType.Private,ChatChannelType.Leaders,ChatChannelType.Group}));Assert.That(capabilities.Channels.Distinct().Count(),Is.EqualTo(capabilities.Channels.Count));Assert.That(capabilities.IdempotencyReceiptRetentionDays,Is.EqualTo(30));});
