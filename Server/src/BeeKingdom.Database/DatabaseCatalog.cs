...
public static class DatabaseCatalog
{
    ...
    [
        ...
        new DatabaseScript(
            ...
            END
            """),
        // News/Actualites bilingual CMS backing the beekingdom-web companion site - one row per
        // article, EN+FR columns side by side (not a JSON blob) so the site can list/sort by
        // publish date directly. See BeeKingdom.News's SqlNewsRepository for the read/write side.
        new DatabaseScript(
            "093_news_articles.sql",
            """
            ...
            END
            """),
        // RAP-OPTIONNEL-COMMUNICATIONS_01: player-created chat groups. ChatGroupInvites is the
        // membership request log (a declined invite stays as a row so the inviter can be told);
        // ChatPreferences holds the cross-device auto-response rule that must apply even while the
        // invited player is offline. Cosmetic chat preferences stay client-side and are NOT here.
        new DatabaseScript(
            "094_chat_groups.sql",
            """
