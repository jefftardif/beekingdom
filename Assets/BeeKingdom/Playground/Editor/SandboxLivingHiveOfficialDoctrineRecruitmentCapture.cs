...
{
    [InitializeOnLoad]
    public static class SandboxLivingHiveOfficialDoctrineRecruitmentCapture
    {
        private const string ScenePath = "Assets/Scenes/LivingHive.unity";
        private const string OutputDirectory =
            "C:/projets/beekingdomgame-master/Docs/Product/Evidence/" +
            "LivingHiveOfficialDoctrineRecruitment";
        private const string ManifestPath =
            OutputDirectory +
            "/LivingHiveOfficialDoctrineRecruitment_CaptureManifest.md";
        private const string Requested =
            "BeeKingdom.OfficialDoctrineRecruitment.Capture.Requested";
        private const string Frames =
            "BeeKingdom.OfficialDoctrineRecruitment.Capture.Frames";
        private const string Captured =
            "BeeKingdom.OfficialDoctrineRecruitment.Capture.Captured";
        private const string Index =
            "BeeKingdom.OfficialDoctrineRecruitment.Capture.Index";
        private const string ConfiguredIndex =
            "BeeKingdom.OfficialDoctrineRecruitment.Capture.ConfiguredIndex";
        ...
        {
            new CaptureSpec(
                "Recrutement doctrinal officiel non configuré FR",
                "LivingHive_OfficialDoctrineRecruitment_" +
                "NotConfigured_FR_390x844.png",
                390,
                844,
                "fr-CA"),
            new CaptureSpec(
                "Official doctrine recruitment not configured EN",
                "LivingHive_OfficialDoctrineRecruitment_" +
                "NotConfigured_EN_1600x900.png",
                1600,
                900,
                "en-US")
        };

        static SandboxLivingHiveOfficialDoctrineRecruitmentCapture()
        {
            if (SessionState.GetBool(Requested, false)) Subscribe();
        }

        [MenuItem(
            "Bee Kingdom/Playground/" +
            "Capture LivingHive Official Doctrine Recruitment Proofs")]
            ...
            try
            {
                    ...
                    new FileInfo(path).Length == 0)
                {
                    ...
                    throw new InvalidOperationException(
                        "Official doctrine recruitment screenshot " +
                ...
                Finish(
                    0,
                    "LivingHive official doctrine recruitment " +
                    "proofs captured.");
            }
        ...
        private static void ConfigureAndApply()
        {
            ...
            HiveViewProductUiPresenter
                .PrepareOfficialDoctrineRecruitmentCaptureForProof(
                    spec.Locale,
        ...
        private static string BuildManifest()
        {
            ...
            builder.AppendLine();
            builder.AppendLine(
                "- Scène: `Assets/Scenes/LivingHive.unity`");
            builder.AppendLine(
                "- État présenté: recrutement doctrinal officiel " +
                "non configuré.");
            builder.AppendLine(
                "- Aucun roster, solde, offre, opération, reçu ou " +
                "succès n’est simulé.");
            builder.AppendLine(
                "- Nature: preuve injectable de mise en page; " +
                "aucune session serveur live.");
            builder.AppendLine(
                "- Appareil: rendu, cache GET protégé, minuteur " +
                "visuel et commande préparée chiffrée.");
            builder.AppendLine(
                "- Serveur: identité, roster, soldes, catalogue, " +
                "UTC, file, révisions et reçus.");
            builder.AppendLine(
                "- Soumission automatique d’une commande " +
                "préparée: `false`");
            builder.AppendLine(
                "- Débit, formation ou claim local: `false`");
            builder.AppendLine(
                "- Terrain 50x50, image de ruche et scènes " +
                "modifiés: `false`");
            builder.AppendLine();
            foreach (CaptureSpec capture in Captures)
            {
