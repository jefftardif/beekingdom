...
{
    [InitializeOnLoad]
    public static class SandboxLivingHiveProgressCapture
    {
        private const string ScenePath = "Assets/Scenes/LivingHive.unity";
        private const string OutputDirectory = "C:/projets/beekingdomgame-master/Docs/Product/Evidence/LivingHiveProgress";
        private const string ManifestPath = OutputDirectory + "/LivingHiveProgress_CaptureManifest.md";
        private const string Requested = "BeeKingdom.HiveProgress.Capture.Requested";
        private const string Frames = "BeeKingdom.HiveProgress.Capture.Frames";
        private const string Captured = "BeeKingdom.HiveProgress.Capture.Captured";
        private const string Index = "BeeKingdom.HiveProgress.Capture.Index";
        private const string ConfiguredIndex = "BeeKingdom.HiveProgress.Capture.ConfiguredIndex";
        ...
        {
            new CaptureSpec("Effectifs persistés mobile FR", "LivingHive_Progress_Army_FR_390x844.png", 390, 844, "army_fr"),
            new CaptureSpec("Niveau de bâtiment persistant paysage EN", "LivingHive_Progress_Building_EN_1600x900.png", 1600, 900, "building_en")
        };

        static SandboxLivingHiveProgressCapture()
        {
            if (SessionState.GetBool(Requested, false)) Subscribe();
        }

        [MenuItem("Bee Kingdom/Playground/Capture LivingHive Progress Proofs")]
        ...
        private static void OnUpdate()
        {
            ...
            try
            {
                ...
                if (!File.Exists(path) || new FileInfo(path).Length == 0)
                {
                    ...
                    throw new InvalidOperationException("Hive progress screenshot was not written: " + path);
                }
                ...
                Finish(0, "LivingHive progress proofs captured.");
            }
            catch (Exception exception)
            {
                Debug.LogError(exception);
                Finish(1, "LivingHive progress capture failed.");
            }
        }
        ...
        private static void ConfigureAndApplyCurrentState()
        {
            ...
            HiveViewProductUiPresenter.PrepareLocalPreviewHiveProgressCaptureForProof(spec.State);
        ...
        private static string BuildManifest()
        {
            ...
            builder.AppendLine("# LivingHive Progress - manifeste de captures");
            builder.AppendLine();
            builder.AppendLine("- Scène: `Assets/Scenes/LivingHive.unity`");
            builder.AppendLine("- États: effectifs restaurés `fr-CA`; bâtiment restauré `en-US`");
            builder.AppendLine("- Appareil: cache local v1, borné, partitionné par profil, non protégé dans cette démo");
            builder.AppendLine("- Serveur: autorité officielle révisionnée; aucune synchronisation simulée");
            builder.AppendLine("- Reprise des files: fusion monotone et idempotente");
