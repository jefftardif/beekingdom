...
{
    public static class WorldMapWave6V3MPreviewScenePlayModeProofHarness
    {
        private const string OutputRoot = "Docs/BuilderA/WorldMapWave6_50x50_V3MPreview/PreviewScenePlayProof";
        private const string SessionRunningKey = "BeeKingdom.WorldMapWave6.V3MPreview.ScenePlayProof.Running";
        private const string ExitPendingKey = "BeeKingdom.WorldMapWave6.V3MPreview.ScenePlayProof.ExitPending";
        private const string ExitCodeKey = "BeeKingdom.WorldMapWave6.V3MPreview.ScenePlayProof.ExitCode";
        ...
        [MenuItem("Bee Kingdom/World Map/Run Wave6 V3M Preview Scene Play Proof")]
        public static void RunWave6V3MPreviewScenePlayProof()
        {
            Directory.CreateDirectory(AbsoluteProjectPath(OutputRoot));
            Evidence.Clear();
            SessionState.SetBool(SessionRunningKey, true);
            SessionState.SetBool(ExitPendingKey, false);
            EditorSceneManager.OpenScene(WorldMapWave6V3MPreviewSceneBuilder.PreviewScenePath);
            EditorApplication.update -= RunWhenReady;
            EditorApplication.update += RunWhenReady;
        ...
        private static void RunWhenReady()
        {
            ...
            try
            {
                ...
                Evidence.Add("preview_scene=" + WorldMapWave6V3MPreviewSceneBuilder.PreviewScenePath);
                Evidence.Add("uses_v3m_preview_runtime_package:" + bootstrap.UsesV3MPreviewRuntimePackageForPlayMode().ToString().ToLowerInvariant());
                Require(bootstrap.UsesV3MPreviewRuntimePackageForPlayMode(), "Preview scene did not enable the V3M runtime package flag.");

                WorldMapMmoFullscreenFoundationBootstrap.Wave6ProofSnapshot initial = bootstrap.CurrentWave6ProofSnapshot();
                Require(initial.ManifestReady, "V3M preview scene Wave6 manifest not ready.");
                Require(initial.VisibleTilesReady, "V3M preview scene visible tiles not ready.");
                Require(string.Equals(bootstrap.CurrentWave6MasterSha256ForProof(), WorldMapWave6StreamingTileProvider.V3MPreviewExpectedMasterSha256, StringComparison.OrdinalIgnoreCase), "Preview scene loaded the wrong Wave6 master SHA-256.");
                ...
                Evidence.Add("VISUAL_FINAL_HD=NO");
        ...
        {
            string receipt = AbsoluteProjectPath(OutputRoot + "/WorldMapWave6_V3MPreviewScene_PlayModeProofReceipt.md");
            var builder = new StringBuilder();
            builder.AppendLine("# WorldMap Wave6 V3M Preview Scene Play Mode Proof");
            builder.AppendLine();
