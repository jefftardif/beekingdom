...
{
    [InitializeOnLoad]
    public static class SandboxLivingHiveProductionForecastCapture
    {
        private const string ScenePath = "Assets/Scenes/LivingHive.unity";
        private const string OutputDirectory = "C:/projets/beekingdomgame-master/Docs/Product/Evidence/LivingHiveProductionForecast";
        private const string ManifestPath = OutputDirectory + "/LivingHiveProductionForecast_CaptureManifest.md";
        private const string Requested = "BeeKingdom.ProductionForecast.Capture.Requested";
        private const string Frames = "BeeKingdom.ProductionForecast.Capture.Frames";
        private const string Captured = "BeeKingdom.ProductionForecast.Capture.Captured";
        private const string Index = "BeeKingdom.ProductionForecast.Capture.Index";
        private const string ConfiguredIndex = "BeeKingdom.ProductionForecast.Capture.ConfiguredIndex";

        private readonly struct CaptureSpec
        {
            public CaptureSpec(string label, string fileName, int width, int height, bool english)
            {
                Label = label;
                FileName = fileName;
                Width = width;
        ...
        {
            new CaptureSpec("Prévision production FR", "LivingHive_ProductionForecast_FR_390x844.png", 390, 844, false),
            new CaptureSpec("Production forecast EN", "LivingHive_ProductionForecast_EN_1600x900.png", 1600, 900, true)
        };

        static SandboxLivingHiveProductionForecastCapture()
        {
            if (SessionState.GetBool(Requested, false)) Subscribe();
        }

        [MenuItem("Bee Kingdom/Playground/Capture LivingHive Production Forecast Proofs")]
        ...
        private static void OnUpdate()
        {
            ...
            try
            {
                ...
                if (!File.Exists(path) || new FileInfo(path).Length == 0)
                {
                    ...
                    throw new InvalidOperationException("Production forecast screenshot was not written: " + path);
                }
                ...
                Finish(0, "LivingHive production forecast proofs captured.");
            }
            catch (Exception exception)
            {
                Debug.LogError(exception);
                Finish(1, "LivingHive production forecast capture failed.");
            }
        }
        ...
        private static void ConfigureAndApplyCurrentState()
        {
            ...
            HiveViewProductUiPresenter.PrepareProductionForecastCaptureForProof(spec.English);
        ...
        private static string BuildManifest()
        {
            ...
            builder.AppendLine("# LivingHive Production Forecast - manifeste de captures");
            builder.AppendLine();
            builder.AppendLine("- Scène: `Assets/Scenes/LivingHive.unity`");
            builder.AppendLine("- Appareil: calcule et affiche le décompte depuis un instantané; aucune autorité économique");
            builder.AppendLine("- Serveur: autorité sur UTC, taux, capacité et quantité en attente");
            builder.AppendLine("- Collecte depuis le panneau: `false`; `Voir` ouvre seulement le bâtiment");
            builder.AppendLine("- Collecte manuelle conservée: `true`");
            ...
            foreach (CaptureSpec capture in Captures)
            {
                ...
                builder.AppendLine("- `" + capture.FileName + "`: `" + dimensions.x + "x" + dimensions.y + "`, locale `" + (capture.English ? "en-US" : "fr-CA") + "`, SHA-256 `" + Sha256(path) + "`");
            }
