...
{
    public static class WorldMapWave5PlayerProofHarness
    {
        ...
        private const string OutputRoot = "Docs/BuilderA/WorldMapWave5_25x25_UnityIntegration/PlayerProof";
        private const string SessionRunningKey = "BeeKingdom.WorldMapWave5.PlayerProof.Running";
        private const string SessionStartedKey = "BeeKingdom.WorldMapWave5.PlayerProof.Started";
        private const string ExitPendingKey = "BeeKingdom.WorldMapWave5.PlayerProof.ExitPending";
        private const string ExitCodeKey = "BeeKingdom.WorldMapWave5.PlayerProof.ExitCode";
        ...
        [MenuItem("Bee Kingdom/World Map/Run Wave5 Player Proof Harness")]
        public static void RunWave5PlayerProofHarness()
        {
            root = AbsoluteProjectPath(OutputRoot);
        ...
        private static void ProcessHarness()
        {
            ...
            try
            {
                ...
                if (!stateApplied)
                {
                    WorldMapMmoFullscreenFoundationBootstrap.Wave5ProofSnapshot before = bootstrap.CurrentWave5ProofSnapshot();
                    Vector2 center = ResolveCenter(spec, before.WorldBounds);
                    bootstrap.ApplyWave5ProofView(center, spec.Zoom);
        ...
        private static Vector2 ResolveCenter(CaptureSpec spec, Rect bounds)
        {
            ...
            Require(safe.width >= 0f && safe.height >= 0f, "Viewport cannot fit inside Wave5 bounds for " + spec.Label);
        ...
        {
            WorldMapMmoFullscreenFoundationBootstrap.Wave5ProofSnapshot state = bootstrap.CurrentWave5ProofSnapshot();
        ...
        private static void CompleteHarness()
        {
            ...
            File.WriteAllText(Path.Combine(root, "WorldMapWave5_PlayerProofTelemetry.json"), BuildTelemetry(metrics, bootstrapHashAfter, ready), new UTF8Encoding(false));
            File.WriteAllText(Path.Combine(root, "WorldMapWave5_PlayerProofReceipt.md"), BuildReceipt(metrics, bootstrapHashAfter, ready), new UTF8Encoding(false));
        ...
        private static int CountProjectedSeams(Texture2D texture, CaptureSample sample, out float worstRatio)
        {
            ...
            for (int index = 1; index < WorldMapWave5StreamingTileProvider.Columns; index++)
            {
                float worldX = bounds.xMin + index * WorldMapWave5StreamingTileProvider.TileSize;
            ...
            for (int index = 1; index < WorldMapWave5StreamingTileProvider.Rows; index++)
            {
                float worldY = bounds.yMin + index * WorldMapWave5StreamingTileProvider.TileSize;
        ...
        private static string BuildTelemetry(List<ImageMetrics> metrics, string bootstrapHashAfter, bool ready)
        {
            ...
            builder.AppendLine("{");
            Json(builder, "schema", "bee-kingdom.world-map.wave5-player-proof.v1", true);
            Json(builder, "utc", DateTime.UtcNow.ToString("O", CultureInfo.InvariantCulture), true);
            Json(builder, "scene", ScenePath, true);
            Json(builder, "source_master_sha256", WorldMapWave5StreamingTileProvider.ExpectedMasterSha256, true);
        ...
        private static string BuildReceipt(List<ImageMetrics> metrics, string bootstrapHashAfter, bool ready)
        {
            ...
            var builder = new StringBuilder();
            builder.AppendLine("# WorldMap Wave5 25x25 Player Proof Receipt");
            builder.AppendLine();
        ...
        private static void DeletePreviousOutputs()
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                if (name.EndsWith(".png", StringComparison.OrdinalIgnoreCase)
                    || name == "WorldMapWave5_PlayerProofTelemetry.json"
                    || name == "WorldMapWave5_PlayerProofReceipt.md"
                    || name == "WorldMapWave5_PlayerProofFailure.txt")
                {
                    File.Delete(files[i]);
                }
            }
        ...
        private static void FailAndExit(string reason)
        {
            ...
            Directory.CreateDirectory(root);
            File.WriteAllText(Path.Combine(root, "WorldMapWave5_PlayerProofFailure.txt"), reason, new UTF8Encoding(false));
        ...
        private readonly struct CaptureSample
        {
            ...
            public readonly WorldMapMmoFullscreenFoundationBootstrap.Wave5ProofSnapshot State;
            ...
            public CaptureSample(CaptureSpec spec, WorldMapMmoFullscreenFoundationBootstrap.Wave5ProofSnapshot state, Rect hudRect, Rect toggleRect, Rect bearScreenRect, string screenshotPath)
            {
                Spec = spec;
                State = state;
                HudRect = hudRect;
