...
{
    [InitializeOnLoad]
    public static class WorldMapWave6SupportCenterNativeAuditPreviewOneShotProofRunner
    {
        private const string RequestFilePath = "Temp/RunWave6SupportCenterNativeAuditPreviewProof.request";
        private const string OutputRoot = "Docs/BuilderA/WorldMapWave6_50x50_SupportCenterNativeAuditPreview/PreviewScenePlayProofLocal";
        private const string RunningKey = "BeeKingdom.WorldMapWave6.SupportCenterNativeAuditPreview.LocalProof.Running";
        private const string PhaseKey = "BeeKingdom.WorldMapWave6.SupportCenterNativeAuditPreview.LocalProof.Phase";
        private const string CaptureFrameKey = "BeeKingdom.WorldMapWave6.SupportCenterNativeAuditPreview.LocalProof.CaptureFrame";
        private static readonly List<string> Evidence = new List<string>();

        private static readonly CaptureSpec[] Captures =
        {
            new CaptureSpec("ROUTE_LOCK_8192_LOCAL_CENTER_C24_R24_Z058", 24, 24, 0.58f),
            new CaptureSpec("ROUTE_LOCK_8192_LOCAL_NORTH_WEST_C16_R16_Z058", 16, 16, 0.58f),
            new CaptureSpec("ROUTE_LOCK_8192_LOCAL_SOUTH_EAST_C40_R40_Z058", 40, 40, 0.58f)
        };

        static WorldMapWave6SupportCenterNativeAuditPreviewOneShotProofRunner()
        {
        ...
        private static void WatchRequestAndRun()
        {
            ...
            {
                Debug.LogWarning($"Could not clear Wave6 support-center proof request: {exception.Message}");
                return;
            }
        ...
        {
            string receipt = AbsoluteProjectPath(OutputRoot + "/WorldMapWave6_SupportCenterNativeAuditPreviewScene_LocalPlayModeProofReceipt.md");
            var builder = new StringBuilder();
            builder.AppendLine("# WorldMap Wave6 Route-Lock 8192 Scale-Bridge Proof Scene Local Play Mode Proof");
            builder.AppendLine();
