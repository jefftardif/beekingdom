...
{
    [InitializeOnLoad]
    public static class SandboxLivingHiveLedgerCapture
    {
        private const string ScenePath = "Assets/Scenes/LivingHive.unity";
        private const string OutputDirectory = "C:/projets/beekingdomgame-master/Docs/Product/Evidence/LivingHiveLedger";
        private const string ManifestPath = OutputDirectory + "/LivingHiveLedger_CaptureManifest.md";
        private const string Requested = "BeeKingdom.Ledger.Capture.Requested";
        private const string Frames = "BeeKingdom.Ledger.Capture.Frames";
        private const string Captured = "BeeKingdom.Ledger.Capture.Captured";
        private const string Index = "BeeKingdom.Ledger.Capture.Index";
        private const string ConfiguredIndex = "BeeKingdom.Ledger.Capture.ConfiguredIndex";
        ...
        {
            new CaptureSpec("LivingHive_Ledger_Portrait_390x844.png", 390, 844),
            new CaptureSpec("LivingHive_Ledger_Landscape_1600x900.png", 1600, 900)
        };

        static SandboxLivingHiveLedgerCapture()
        {
            if (SessionState.GetBool(Requested, false)) Subscribe();
        }

        [MenuItem("Bee Kingdom/Playground/Capture LivingHive Ledger Proofs")]
        ...
        private static void OnUpdate()
        {
            ...
            try
            {
                ...
                if (!File.Exists(path) || new FileInfo(path).Length == 0)
                {
                    ...
                    throw new InvalidOperationException("Ledger screenshot was not written: " + path);
                }
                ...
                Finish(0, "LivingHive Ledger proofs captured.");
            }
            catch (Exception exception)
            {
                Debug.LogError(exception);
                Finish(1, "LivingHive Ledger capture failed.");
            }
        }
        ...
        private static void ConfigureAndApply()
        {
            ...
            CaptureSpec spec = Current();
            TrySetGameViewSize(spec.Width, spec.Height, "Ledger " + spec.Width + "x" + spec.Height);
            Screen.SetResolution(spec.Width, spec.Height, false);
            HiveViewProductUiPresenter.PrepareHiveLedgerCaptureForProof();
        ...
        private static string BuildManifest()
        {
            ...
            builder.AppendLine("# LivingHive Ledger - manifeste de captures");
            builder.AppendLine();
            builder.AppendLine("- Scène: `Assets/Scenes/LivingHive.unity`");
            builder.AppendLine("- Autorité affichée: `aperçu local non officiel`; autorité officielle: `serveur`");
            builder.AppendLine("- Collecte directe depuis le panneau: `false`");
            builder.AppendLine("- Navigation: `Sac -> Voir -> bâtiment -> collecte manuelle`");
            builder.AppendLine("- Cibles Voir et fermeture: `44 px` minimum");
            builder.AppendLine("- Engagement montré: `foraging_routes_i`, 240 miel + 90 pollen");
