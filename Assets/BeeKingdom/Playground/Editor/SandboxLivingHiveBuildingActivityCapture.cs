...
{
    [InitializeOnLoad]
    public static class SandboxLivingHiveBuildingActivityCapture
    {
        private const string ScenePath = "Assets/Scenes/LivingHive.unity";
        private const string OutputDirectory = "C:/projets/beekingdomgame-master/Docs/Product/Evidence/LivingHiveBuildingActivity";
        private const string ManifestPath = OutputDirectory + "/LivingHiveBuildingActivity_CaptureManifest.md";
        private const string Requested = "BeeKingdom.BuildingActivity.Capture.Requested";
        private const string Frames = "BeeKingdom.BuildingActivity.Capture.Frames";
        private const string Captured = "BeeKingdom.BuildingActivity.Capture.Captured";
        private const string Index = "BeeKingdom.BuildingActivity.Capture.Index";
        private const string ConfiguredIndex = "BeeKingdom.BuildingActivity.Capture.ConfiguredIndex";
        ...
        {
            new CaptureSpec("Activité bâtiment FR", "LivingHive_BuildingActivity_FR_390x844.png", 390, 844, true),
            new CaptureSpec("Building activity EN", "LivingHive_BuildingActivity_EN_1600x900.png", 1600, 900, false)
        };

        static SandboxLivingHiveBuildingActivityCapture()
        {
            if (SessionState.GetBool(Requested, false)) Subscribe();
        }

        [MenuItem("Bee Kingdom/Playground/Capture LivingHive Building Activity Proofs")]
        ...
        private static void OnUpdate()
        {
            ...
            try
            {
                ...
                if (!File.Exists(path) || new FileInfo(path).Length == 0)
                {
                    ...
                    throw new InvalidOperationException("Building activity screenshot was not written: " + path);
                }
                ...
                Finish(0, "LivingHive building activity proofs captured.");
            }
            catch (Exception exception)
            {
                Debug.LogError(exception);
                Finish(1, "LivingHive building activity capture failed.");
            }
        }
        ...
        private static void ConfigureAndApplyCurrentState()
        {
            ...
            HiveViewProductUiPresenter.PrepareBuildingActivityCaptureForProof(spec.Portrait);
        ...
        private static string BuildManifest()
        {
            ...
            builder.AppendLine("# LivingHive Building Activity - manifeste de captures");
            builder.AppendLine();
            builder.AppendLine("- Scène: `Assets/Scenes/LivingHive.unity`");
            builder.AppendLine("- Portrait: couvain, français, mouvement réduit statique");
            builder.AppendLine("- Paysage: façonnage de cire, anglais, mouvement ambiant");
            builder.AppendLine("- Appareil: rendu et préférences visuelles seulement");
            builder.AppendLine("- Serveur: autorité sur les stocks, minuteries et états officiels");
            builder.AppendLine("- Mutation de ressources ou de progression par l'animation: `false`");
