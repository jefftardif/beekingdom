...
{
    [InitializeOnLoad]
    public static class SandboxQA065ReserveClosureCapture
    {
        private const string ScenePath = "Assets/Scenes/SandboxPlayground.unity";
        private const string OutputDirectory = "C:/projets/beekingdom/prompt_demo/rapports/DEMO-066_QA065ReserveClosure_Source";
        private const string ManifestPath = OutputDirectory + "/DEMO-066_QA065ReserveClosure_Manifest.md";
        private const string ReportPath = "C:/projets/beekingdom/prompts_codex/rapports/BuilderA_QA065ReserveClosure_Report.md";
        private const string StateRequested = "BeeKingdom.Playground.QA065ReserveClosure.Requested";
        private const string StateFrames = "BeeKingdom.Playground.QA065ReserveClosure.Frames";
        private const string StateCaptured = "BeeKingdom.Playground.QA065ReserveClosure.Captured";
        private const string StateIndex = "BeeKingdom.Playground.QA065ReserveClosure.Index";
        ...
        {
            new CaptureSpec("Close button exercised", "QA065_01_CloseButtonClosedPanel_1280x720.png", 1280, 720, "honey_storage", "close_exercised", Vector2.zero, 1.08f),
            new CaptureSpec("Anti double upgrade cost", "QA065_02_DoubleUpgradeGuard_1280x720.png", 1280, 720, "honey_storage", "double_upgrade_guard", Vector2.zero, 1.08f),
            new CaptureSpec("Anti double training queue", "QA065_03_DoubleTrainingGuard_1280x720.png", 1280, 720, "guard_post", "double_training_guard", new Vector2(-18f, 10f), 1.12f),
            new CaptureSpec("Disabled reason visible", "QA065_04_DisabledReasonReadable_1280x720.png", 1280, 720, "honey_storage", "upgrade_blocked", Vector2.zero, 1.08f),
            new CaptureSpec("Phone portrait comfort", "QA065_05_PhonePortraitReadable_390x844.png", 390, 844, "guard_post", "training_running", new Vector2(-160f, 48f), 1.16f),
            new CaptureSpec("Gesture one finger pan proof", "QA065_06_OneFingerPanHudFixed_1280x720.png", 1280, 720, "administration_core", "gesture_pan_proof", new Vector2(42f, -18f), 1.08f),
            new CaptureSpec("Gesture two finger pinch proof", "QA065_07_TwoFingerPinchHudFixed_1280x720.png", 1280, 720, "administration_core", "gesture_pinch_proof", new Vector2(42f, -18f), 1.18f),
            new CaptureSpec("UI blocks hive gesture proof", "QA065_08_UiBlocksHiveGesture_1280x720.png", 1280, 720, "honey_storage", "ui_gesture_blocked", Vector2.zero, 1.08f)
        };

        static SandboxQA065ReserveClosureCapture()
        {
        ...
        [MenuItem("Bee Kingdom/Playground/Capture DEMO-066 QA065 Reserve Closure")]
        public static void CaptureQA065ReserveClosure()
        {
        ...
        private static void OnPlayModeUpdate()
        {
            ...
            try
            {
                ...
                if (!File.Exists(path) || new FileInfo(path).Length == 0)
                {
                    ...
                    throw new InvalidOperationException("DEMO-066 screenshot was not written: " + path);
                }
                ...
                Debug.Log("DEMO-066 QA065 reserve closure captured.");
            ...
            catch (Exception exception)
            {
                SessionState.SetBool(StateRequested, false);
                EditorApplication.update -= OnPlayModeUpdate;
                Debug.LogError("DEMO-066 QA065 reserve closure capture failed: " + exception);
        ...
        private static string BuildManifest()
        {
            ...
            builder.AppendLine("# DEMO-066 QA065 Reserve Closure Manifest");
            builder.AppendLine();
            builder.AppendLine("## Scope");
            builder.AppendLine();
            builder.AppendLine("- Surface: `Ruche jouable preview`");
            ...
            builder.AppendLine();
            builder.AppendLine("## Gesture Telemetry");
            builder.AppendLine();
            ...
            builder.AppendLine();
            builder.AppendLine("READY_FOR_DEMO_066 = YES");
        ...
        {
            var builder = new StringBuilder();
            builder.AppendLine("# Builder-A QA065 Reserve Closure Report");
            builder.AppendLine();
            ...
            builder.AppendLine();
            builder.AppendLine("Fermeture ciblee des reserves QA DEMO-065 sur la Ruche jouable preview. Aucun travail carte monde, serveur live, sauvegarde officielle, economie officielle ou armee persistante officielle.");
            builder.AppendLine();
            builder.AppendLine("## Reserves QA fermees");
            builder.AppendLine();
            builder.AppendLine("* Close button: bouton Fermer exerce, panneau compacte, bouton Ouvrir visible.");
            builder.AppendLine("* Anti double cout: Ameliorer bloque la repetition; commit count expose.");
            builder.AppendLine("* Anti double training: entrainement bloque la repetition; queue et commit count exposes.");
            builder.AppendLine("* Disabled reason: raison remontee au-dessus de l'action, lisible dans le panneau normal.");
            builder.AppendLine("* Panneau droit: densite reduite et blocs action/cout/progression/file separes.");
            builder.AppendLine("* Portrait telephone: preuve 390x844 regeneree avec panneau compact.");
            builder.AppendLine("* Gestes: preuve one-finger pan, two-finger pinch, fixed UI blocks et UI blocks hive gesture.");
            builder.AppendLine("* Checks deterministes: cout unique, increment niveau, queue training, increment troupes.");
            builder.AppendLine();
            builder.AppendLine("## Fichiers crees");
            builder.AppendLine();
            builder.AppendLine("* C:/projets/beekingdomgame-master/Assets/BeeKingdom/Playground/Editor/SandboxQA065ReserveClosureCapture.cs");
            builder.AppendLine();
            ...
            foreach (CaptureSpec capture in Captures) builder.AppendLine("* " + capture.Label + ": `" + PathFor(capture) + "`");
            builder.AppendLine();
            builder.AppendLine("## Limitations non-live");
            builder.AppendLine();
            ...
            builder.AppendLine("* Aucune economie ou armee persistante officielle.");
            builder.AppendLine();
            builder.AppendLine("## READY_FOR_DEMO_066");
            builder.AppendLine();
        ...
        private static void TrySetGameViewSize(int width, int height, string label)
        {
            ...
            {
                Debug.LogWarning("Unable to force DEMO-066 Game View size " + width + "x" + height + ": " + exception.Message);
            }
        }
    }
}
