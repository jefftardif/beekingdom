...
{
    [InitializeOnLoad]
    public static class SandboxLivingHiveBuildingUpgradeCapture
    {
        private const string ScenePath = "Assets/Scenes/LivingHive.unity";
        private const string OutputDirectory = "C:/projets/beekingdomgame-master/Docs/Product/Evidence/LivingHiveBuildingUpgrade";
        private const string ManifestPath = OutputDirectory + "/LivingHiveBuildingUpgrade_CaptureManifest.md";
        private const string Requested = "BeeKingdom.BuildingUpgrade.Capture.Requested";
        private const string Frames = "BeeKingdom.BuildingUpgrade.Capture.Frames";
        private const string Captured = "BeeKingdom.BuildingUpgrade.Capture.Captured";
        private const string Index = "BeeKingdom.BuildingUpgrade.Capture.Index";
        private const string ConfiguredIndex = "BeeKingdom.BuildingUpgrade.Capture.ConfiguredIndex";
        ...
        private sealed class HonestNotConfiguredController : IHiveBuildingUpgradePanelController
        {
        ...
        {
            new CaptureSpec("Construction officielle non configurée FR",
                "LivingHive_BuildingUpgrade_NotConfigured_FR_390x844.png", 390, 844, "fr-CA"),
            new CaptureSpec("Official construction not configured EN",
                "LivingHive_BuildingUpgrade_NotConfigured_EN_1600x900.png", 1600, 900, "en-US")
        };

        static SandboxLivingHiveBuildingUpgradeCapture()
        {
            if (SessionState.GetBool(Requested, false)) Subscribe();
        }

        [MenuItem("Bee Kingdom/Playground/Capture LivingHive Building Upgrade Proofs")]
        ...
        private static void OnUpdate()
        {
            ...
            try
            {
                ...
                if (!File.Exists(path) || new FileInfo(path).Length == 0)
                {
                    ...
                    throw new InvalidOperationException("Building upgrade screenshot was not written: " + path);
                }
                ...
                Finish(0, "LivingHive building upgrade proofs captured.");
            }
            catch (Exception exception)
            {
                Debug.LogError(exception);
                Finish(1, "LivingHive building upgrade capture failed.");
            }
        }
        ...
        private static void ConfigureAndApplyCurrentState()
        {
            ...
            HiveViewProductUiPresenter.ConfigureBuildingUpgradeControllerForRuntime(new HonestNotConfiguredController());
        ...
        private static string BuildManifest()
        {
            ...
            builder.AppendLine("# LivingHive Building Upgrade - manifeste de captures");
            builder.AppendLine();
            builder.AppendLine("- Scène: `Assets/Scenes/LivingHive.unity`");
            builder.AppendLine("- État présenté: construction officielle non configurée.");
            builder.AppendLine("- Aucun coût, niveau, minuteur, succès, ressource ou reçu n’est simulé.");
            ...
            builder.AppendLine("- Serveur: autorité sur catalogue, coût, UTC, file, niveau, révision et reçu.");
