...
{
    public static class WorldMapWave6V3OReducedAuditPreviewScenePlayModeProofHarness
    {
        private const string OutputRoot = "Docs/BuilderA/WorldMapWave6_50x50_V3OReducedAuditPreview/PreviewScenePlayProof";
        private const string SessionRunningKey = "BeeKingdom.WorldMapWave6.V3OReducedAuditPreview.ScenePlayProof.Running";
        private const string ExitPendingKey = "BeeKingdom.WorldMapWave6.V3OReducedAuditPreview.ScenePlayProof.ExitPending";
        private const string ExitCodeKey = "BeeKingdom.WorldMapWave6.V3OReducedAuditPreview.ScenePlayProof.ExitCode";
        private const string PhaseKey = "BeeKingdom.WorldMapWave6.V3OReducedAuditPreview.ScenePlayProof.Phase";
        private const string CaptureFrameKey = "BeeKingdom.WorldMapWave6.V3OReducedAuditPreview.ScenePlayProof.CaptureFrame";
        private const int RealWindowMinRow = 21;
        private const int RealWindowMaxRow = 28;
        private const int RealWindowMinColumn = 21;
        private const int RealWindowMaxColumn = 28;
        private static readonly List<string> Evidence = new List<string>();

        private static readonly CaptureSpec[] Captures =
        {
            new CaptureSpec("V3O_8X8_CENTER_C24_R24_Z058", 24, 24, 0.58f),
            new CaptureSpec("V3O_8X8_NORTH_WEST_C22_R22_Z058", 22, 22, 0.58f),
            new CaptureSpec("V3O_8X8_SOUTH_EAST_C27_R27_Z058", 27, 27, 0.58f)
        };
        ...
        [MenuItem("Bee Kingdom/World Map/Run Wave6 V3O Reduced Audit Preview Scene Play Proof")]
        public static void RunWave6V3OReducedAuditPreviewScenePlayProof()
        {
            ...
            EditorSceneManager.OpenScene(WorldMapWave6V3OReducedAuditPreviewSceneBuilder.PreviewScenePath);
            EditorApplication.update -= RunWhenReady;
            EditorApplication.update += RunWhenReady;
        ...
        private static void RunWhenReady()
        {
            ...
            try
            {
                if (Evidence.Count == 0)
                {
                    ...
                    Evidence.Add("preview_scene=" + WorldMapWave6V3OReducedAuditPreviewSceneBuilder.PreviewScenePath);
                    Evidence.Add("uses_v3o_reduced_audit_preview_runtime_package:" + bootstrap.UsesV3OReducedAuditPreviewRuntimePackageForPlayMode().ToString().ToLowerInvariant());
                    Require(bootstrap.UsesV3OReducedAuditPreviewRuntimePackageForPlayMode(), "Preview scene did not enable the V3O reduced audit runtime package flag.");
                    Evidence.Add("real_v3o_window_rows:" + RealWindowMinRow.ToString(CultureInfo.InvariantCulture) + "-" + RealWindowMaxRow.ToString(CultureInfo.InvariantCulture));
                    Evidence.Add("real_v3o_window_columns:" + RealWindowMinColumn.ToString(CultureInfo.InvariantCulture) + "-" + RealWindowMaxColumn.ToString(CultureInfo.InvariantCulture));
                }
                ...
                if (phase >= Captures.Length)
                {
                    ...
                    Evidence.Add("REDUCED_8X8_UNITY_AUDIT_ONLY=YES");
        ...
        private static void ValidateView(WorldMapMmoFullscreenFoundationBootstrap bootstrap, CaptureSpec spec)
        {
            ...
            Require(string.Equals(bootstrap.CurrentWave6MasterSha256ForProof(), WorldMapWave6StreamingTileProvider.V3OReducedAuditPreviewExpectedMasterSha256, StringComparison.OrdinalIgnoreCase), "Preview scene loaded the wrong V3O reduced audit SHA-256.");
        ...
        {
            string receipt = AbsoluteProjectPath(OutputRoot + "/WorldMapWave6_V3OReducedAuditPreviewScene_PlayModeProofReceipt.md");
            var builder = new StringBuilder();
            builder.AppendLine("# WorldMap Wave6 V3O Reduced Audit Preview Scene Play Mode Proof");
            builder.AppendLine();
