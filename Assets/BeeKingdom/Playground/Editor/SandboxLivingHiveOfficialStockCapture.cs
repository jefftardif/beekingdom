...
{
    [InitializeOnLoad]
    public static class SandboxLivingHiveOfficialStockCapture
    {
        private const string ScenePath = "Assets/Scenes/LivingHive.unity";
        private const string OutputDirectory =
            "C:/projets/beekingdomgame-master/Docs/Product/Evidence/LivingHiveOfficialStock";
        private const string ManifestPath =
            OutputDirectory + "/LivingHiveOfficialStock_CaptureManifest.md";
        private const string Requested =
            "BeeKingdom.OfficialStock.Capture.Requested";
        private const string Frames =
            "BeeKingdom.OfficialStock.Capture.Frames";
        private const string Captured =
            "BeeKingdom.OfficialStock.Capture.Captured";
        private const string Index =
            "BeeKingdom.OfficialStock.Capture.Index";
        private const string ConfiguredIndex =
            "BeeKingdom.OfficialStock.Capture.ConfiguredIndex";
        ...
        private sealed class HonestNotConfiguredController :
            IHiveStockPanelController
        {
            private readonly HiveStockScreenModel model =
                HiveStockPresentation.NotConfigured();
        ...
        {
            new CaptureSpec(
                "Sac et stocks officiel non configuré FR",
                "LivingHive_OfficialStock_NotConfigured_FR_390x844.png",
                390,
                844,
                "fr-CA"),
            new CaptureSpec(
                "Official hive stock not configured EN",
                "LivingHive_OfficialStock_NotConfigured_EN_1600x900.png",
                1600,
                900,
                "en-US")
        };

        static SandboxLivingHiveOfficialStockCapture()
        {
            if (SessionState.GetBool(Requested, false)) Subscribe();
        }

        [MenuItem(
            "Bee Kingdom/Playground/Capture LivingHive Official Stock Proofs")]
            ...
            try
            {
                ...
                if (!File.Exists(path) || new FileInfo(path).Length == 0)
                {
                    ...
                    throw new InvalidOperationException(
                        "Official hive stock screenshot was not written: " +
                        path);
                }
                ...
                Finish(0, "LivingHive official stock proofs captured.");
            }
            catch (Exception exception)
            {
                Debug.LogError(exception);
                Finish(1, "LivingHive official stock capture failed.");
            }
        }
        ...
        private static void ConfigureAndApplyCurrentState()
        {
            ...
            HiveViewProductUiPresenter.PrepareOfficialHiveStockCaptureForProof(
                spec.Locale);
            HiveViewProductUiPresenter.ConfigureHiveStockControllerForRuntime(
        ...
        private static string BuildManifest()
        {
            ...
            builder.AppendLine(
                "# LivingHive Official Stock - manifeste de captures");
            builder.AppendLine();
            builder.AppendLine("- Scène: `Assets/Scenes/LivingHive.unity`");
            builder.AppendLine(
                "- État présenté: stock officiel non configuré.");
            builder.AppendLine(
                "- Aucun montant, capacité, population, engagement, statut ou succès n’est simulé.");
            builder.AppendLine(
                "- Nature: preuve injectable de mise en page; aucune session serveur live.");
            builder.AppendLine(
                "- Appareil: rendu et cache protégé en lecture seule.");
            builder.AppendLine(
                "- Serveur: autorité sur stocks, capacités, population disponible, recherches terminées, engagements, UTC et révision.");
            builder.AppendLine(
                "- Collecte ou mutation disponible dans ces preuves: `false`");
            builder.AppendLine(
